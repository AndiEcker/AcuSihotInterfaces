...
Salesforce (source) and then compare the found clients with assicated clients on the Acumen (destination) system. 
...
The AssServer is a server application that is providing a web-service that listens and connects to our Sihot system
for to propagate the following live actions done within Sihot:
...
fully implemented within Salesforce) also be propagated onto our Salesforce system.
