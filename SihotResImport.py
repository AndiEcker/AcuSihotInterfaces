...
if lastRt.startswith('@'):
    ...
    cae.shutdown(4)
error_msg = cae.set_config(last_rt_prefix + 'lastRt', '@' + str(datetime.datetime.now()))
if error_msg:
    uprint(error_msg)
...
def run_import(acu_user, acu_password):
    ...
    uprint('Acumen Usr/DSN:', acu_user, cae.get_option('acuDSN'))
...
file_count = len((glob.glob(cae.get_option('tciPath')) if cae.get_option('tciPath') else []) +
                 (glob.glob(cae.get_option('rciPath')) if cae.get_option('rciPath') else []))
if not cae.get_config('acuUser') or not cae.get_config('acuPassword'):
    run_mode = RUN_MODE_UI
    ...
    class SihotResImportApp(App):
        ...
        def run_import(self):
            ...
            run_import(self.acu_user, self.user_password)
