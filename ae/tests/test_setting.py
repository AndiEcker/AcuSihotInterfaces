...
class TestLiteral:
    def test_init(self):
        ...
        s = Literal(literal_or_value='test_val')
        assert isinstance(s.value, str)
        assert s.value == 'test_val'

    def test_bool_values(self):
        bs = 'True'
        s = Literal(literal_or_value=bs, value_type=bool)
        assert isinstance(s.value, bool)
        assert s.value is True

        bn = 1
        s = Literal(literal_or_value=bn, value_type=bool)
        assert isinstance(s.value, bool)
        assert s.value is True

    def test_byte_values(self):
        bs = b'TEST'
        s = Literal(literal_or_value=bs)
        assert isinstance(s.value, bytes)
        assert s.value == bs

        s = Literal(literal_or_value=bs, value_type=str)
        assert isinstance(s.value, str)
        assert s.value == 'TEST'

    def test_date_values(self):
        ds = '2020-12-24'
        s = Literal(literal_or_value=ds, value_type=datetime.date)
        ...
        s = Literal(literal_or_value=dts, value_type=datetime.datetime)
