...
from console_app import ConsoleApp, Progress, uprint, MAIN_SECTION_DEF
...
last_run = last_check = startup - command_interval
...
while True:
    ...
    run_msg = "run {} of {} with {} errors.".format(run_starts, run_ends, err_count)
    ...
    try:
        while time.monotonic() < last_check + check_interval:
        ...
        err_msg = BREAK_PREFIX + ' while waiting for next check in ' \
                  + str(last_check + check_interval - time.monotonic()) + ' seconds'
    ...
    try:
        while time.monotonic() < last_run + command_interval:
            time.sleep(1)       # allow to process/raise KeyboardInterrupt within 1 second
    except KeyboardInterrupt:
        err_msg = BREAK_PREFIX + ' while waiting for next command schedule in ' \
                  + str(last_run + command_interval - time.monotonic()) + ' seconds'
        continue            # first notify, then break in next loop because auf BREAK_PREFIX
    ...
    try:
        ...
        err_msg = str(run_starts) + '. run timed out - current time=' + str(time.monotonic()) \
