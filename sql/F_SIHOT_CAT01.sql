create or replace function LOBBY.SIHOT_CAT(pcAptOrGenAtRs IN varchar2)  -- either AP_CODE  or  AT_GENERIC@AT_RSREF[_AFT_CODE]  or  'RU' || RU_CODE
...
BEGIN
  if substr(pcAptOrGenAtRs, 1, 2) = 'RU' then
    ...
    if lnPos > 0 then
      ...
      lnPos := instr(lcResort, '_');
      if lnPos > 0 then
        lcAftSuffix := substr(lcResort, lnPos);
        lcResort := substr(lcResort, 1, lnPos - 1);
        open  cLU_AFT_CATS;
        fetch cLU_AFT_CATS into lcSihotCat;
        close cLU_AFT_CATS;
      end if;
