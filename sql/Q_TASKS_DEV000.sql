...
select * from t_ap where ap_code = '1221'
...
select * from T_RUL l
 where not exists (select NULL from T_RUL c where c.RUL_PRIMARY = l.RUL_PRIMARY and c.RUL_CODE > l.RUL_CODE)
   ...
   and RUL_CODE = 4273384;

select * from t_ru where ru_code = 320138 --338524
...
select V_ACU_RES_LOG.*
     , V_ACU_RES_DATA.*
  from V_ACU_RES_LOG
       left outer join T_RU V_ACU_RES_DATA on RUL_PRIMARY = RU_CODE
 where ( RU_CODE is NULL or RU_FROM_DATE >= trunc(sysdate) or RUL_SIHOT_ROOM is not NULL )                             -- only migrate past reservations if room was assigned
   and ( (RUL_ACTION <> 'DELETE' and nvl(RU_STATUS, 0) <> 120) or RUL_SIHOT_OBJID is not NULL)  -- sync only res deletions/cancelations when res got already migrated/synced into SIHOT
   -- filtering hotels - since having RUL_SIHOT_HOTEL the RUL_CHANGES hotel filter is no longer needed
   --and ( exists (select NULL from resorts where RS_CODE = nvl(RU_RESORT, '_'))
   --   or ( RUL_ACTION = 'DELETE' and exists (select NULL from resorts where RS_CODE = substr(RUL_CHANGES, instr(RUL_CHANGES, 'RU_RESORT (') + 11, 3) ) ) )
   ...
   --   or ( RUL_ACTION = 'DELETE' and exists (select NULL from resoccs where RO_CODE = substr(RUL_CHANGES, instr(RUL_CHANGES, 'RU_ROREF (') + 10, 2) ) ) )
   -- ignoring reservation requests (and changes) with stays before 2012
   --and (RU_CODE is NULL or RU_FROM_DATE + RU_DAYS > DATE'2012-01-01')
   and RUL_CODE = 4273384;
...
select RU_CODE
  ...
  left outer join V_ACU_CD_DATA on RU_CDREF = CD_CODE
  where ru_code = 338524


-- 17:42==72k, 10:37(+17h)==40k(-32k), 8:30(+22h)==1k(-39k)
select count(*) from v_acu_res_unsynced
