...
BEGIN
  ...
  if pcAction in ('CO', 'RM') then
    update T_ARO set ARO_TIMEOUT = sysdate,
     ...
     where ARO_STATUS = 300 and ARO_APREF = nvl(pcOldApt, pcApt) and trunc(sysdate) between ARO_EXP_DEPART - 2 and ARO_EXP_DEPART;
  end if;        
  if pcAction in ('CI', 'RM') then
    update T_ARO set ARO_TIMEIN = sysdate,
                     ARO_STATUS = case when pcAction = 'RM' then 330 else 300 end
     where ARO_STATUS = 200 and ARO_APREF = pcApt and trunc(sysdate) between ARO_EXP_ARRIVE and ARO_EXP_ARRIVE + 2;
  end if;
END
/*
